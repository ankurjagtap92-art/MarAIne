...
// Register Chart.js (only on client, but safe)
...
// Types
interface DashboardStats { totalRoutes: number; fuelSaved: number; riskScore: number; activeVessels: number; }
interface Activity { id: string; type: "route_analyzed" | "vessel_added" | "route_updated"; description: string; created_at: string; }
interface Port { id: string; name: string; unlocode: string; latitude: number; longitude: number; }
interface RouteWithWaypoints { id: string; origin_port: string; destination_port: string; waypoints: { lat: number; lon: number }[]; }
...
export default function DashboardContent() {
  ...
  const [stats, setStats] = useState<DashboardStats>({ totalRoutes: 24, fuelSaved: 18.5, riskScore: 12, activeVessels: 7 });
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        setError("");
        try { const res = await api.get("/api/v1/dashboard/stats"); setStats(res.data); } catch { setStats({ totalRoutes: 5, fuelSaved: 12.5, riskScore: 18, activeVessels: 3 }); }
        try { const res = await api.get("/api/v1/activities/recent?limit=5"); setActivities(res.data); } catch { setActivities([{ id: "1", type: "route_analyzed", description: "Route analyzed: Mumbai → Singapore", created_at: new Date().toISOString() }]); }
        try { const res = await api.get("/api/v1/ports"); setPorts(res.data); } catch { setPorts([{ id: "1", name: "Mumbai", unlocode: "INBOM", latitude: 18.94, longitude: 72.83 }, { id: "2", name: "Singapore", unlocode: "SGSIN", latitude: 1.29, longitude: 103.85 }]); }
        try {
          const res = await api.get("/api/v1/routes");
          const routesWithGeo = res.data.map((r: any) => {
            ...
            if (r.options && r.options.length > 0) {
              ...
              if (firstOption.waypoints && firstOption.waypoints.length > 0) {
                waypoints = firstOption.waypoints.map((w: any) => ({ lat: w.latitude, lon: w.longitude }));
              }
            }
            return { id: r.id, origin_port: r.origin_port, destination_port: r.destination_port, waypoints };
          });
          setRoutesWithWaypoints(routesWithGeo);
        } catch { setRoutesWithWaypoints([]); }
      } catch (err) {
        ...
        setActivities([{ id: "1", type: "route_analyzed", description: "Route analyzed: Mumbai → Singapore", created_at: new Date().toISOString() }]);
        setPorts([{ id: "1", name: "Mumbai", unlocode: "INBOM", latitude: 18.94, longitude: 72.83 }, { id: "2", name: "Singapore", unlocode: "SGSIN", latitude: 1.29, longitude: 103.85 }]);
  ...
  const chartData = { labels: ["Container", "Tanker", "Bulk", "Other"], values: [4, 3, 2, 1] };
  const handleLogout = () => { logout(); router.push("/"); };
  const handleShare = async () => {
    try { await navigator.clipboard.writeText(window.location.href); alert("✅ Dashboard link copied to clipboard!"); } catch { alert("Could not copy link."); }
  };
  ...
  return (
    <ProtectedRoute>
      <div className="flex min-h-screen bg-[#060b1a]">
        {/* Sidebar – copy from your original page.tsx (unchanged) */}
        <aside className={`fixed top-0 left-0 z-50 h-full w-64 border-r border-[#1c2b45] bg-[#0a1628]/80 backdrop-blur-xl transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
          ...
          <nav className="px-4 py-6 space-y-1">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${isActive ? "bg-[#7c5cff]/10 text-[#7c5cff] shadow-[0_0_20px_rgba(124,92,255,0.1)]" : "text-ink-secondary hover:bg-white/5 hover:text-ink-primary"}`}>
          ...
          <header className="sticky top-0 z-40 border-b border-white/5 bg-[#0a1628]/80 backdrop-blur-xl px-6 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button onClick={() => setShowNotifications(!showNotifications)} className="relative h-9 w-9 rounded-full border border-white/10 flex items-center justify-center text-ink-secondary hover:text-white hover:border-white/20 transition">
                  <IconBell className="h-4 w-4" />
                  <span className="absolute -top-0.5 -right-0.5 h-4 w-4 rounded-full bg-status-danger text-[10px] font-bold text-white flex items-center justify-center">{notifications.length}</span>
                </button>
                {showNotifications && (
          ...
          <div className="dashboard-shell" suppressHydrationWarning>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
              {[
                ...
                <StatCard key={i} icon={<stat.icon className="h-5 w-5" />} value={stat.value} label={stat.label} accent={stat.color as any} change={stat.change} trend={stat.trend as any} delay={i * 0.1} />
              ))}
            </div>
